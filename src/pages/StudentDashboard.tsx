    ...
    <div className="min-h-screen bg-background">
      <nav className="sticky top-0 z-50 border-b border-border bg-card/80 backdrop-blur-xl">
        <div className="container mx-auto flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="ml-2 rounded-full bg-primary/10 px-3 py-0.5 text-xs font-semibold text-primary">Student</span>
          </div>
      ...
      <main className="container mx-auto max-w-3xl px-6 py-12">
        <div className="mb-10">
          ...
          <p className="mt-2 text-muted-foreground">Select a subject, upload your answer script, and get instant AI evaluation.</p>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {SUBJECTS.map((subject) => (
              <button key={subject} onClick={() => { setSelectedSubject(subject); setResult(null); setAnswerScript(null); }}
                className={`rounded-xl border-2 p-4 text-left transition-all ${selectedSubject === subject ? "border-primary bg-primary/5 shadow-sm" : "border-border hover:border-muted-foreground/30 hover:bg-accent/50"}`}>
                <FileText className={`h-5 w-5 mb-2 ${selectedSubject === subject ? "text-primary" : "text-muted-foreground"}`} />
                <p className={`text-sm font-medium ${selectedSubject === subject ? "text-primary" : "text-foreground"}`}>{subject}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        {selectedSubject && !result && !loading && (
          <div className="space-y-6 animate-in fade-in-0 slide-in-from-bottom-4 duration-300">
            <h2 className="text-sm font-semibold text-foreground">Upload Answer Script — {selectedSubject}</h2>
            <label htmlFor="answer-script" className="group flex cursor-pointer flex-col items-center gap-3 rounded-xl border-2 border-dashed border-border bg-card px-6 py-12 transition-all hover:border-primary/40 hover:bg-primary/5">
              ...
              {answerScript ? <span className="text-xs text-primary font-medium truncate max-w-full">{answerScript.name}</span> : <span className="text-xs text-muted-foreground">Click to browse · PDF only</span>}
              <input id="answer-script" type="file" accept=".pdf" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) setAnswerScript(f); }} />
            </label>
            <Button onClick={handleEvaluate} disabled={!answerScript} size="lg" className="w-full font-semibold">Start Evaluation</Button>
          </div>
        )}
        ...
        {result && !loading && (
          <section className="mt-8">
            ...
            <div className="mt-6">
              <Button variant="outline" onClick={() => { setResult(null); setAnswerScript(null); }} className="w-full">Evaluate Another Script</Button>
            </div>
          </section>
        )}
      </main>
